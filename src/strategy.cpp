...
// Controle de stamina, estratégias
// Economizar stamina para um possível fim de jogo. 
// 
...
Strategy::get_normal_dash_power( const WorldModel & wm )
{
    ...
    // se ta sem stamina, vou usar a extra.
    if ( wm.self().staminaModel().capacityIsEmpty() )
    {
        // ver qual o menor, o valor pra um dash ou a stamina extra.
        return std::min( ServerParam::i().maxDashPower(),
                         wm.self().stamina() + wm.self().playerType().extraStamina() );
    }

    // ciclos para chegar na bola. 
    ...
    //Menor que 55% do máximo, entrar em modo de recuperação.
    else if ( wm.self().stamina() < ServerParam::i().staminaMax() * 0.55 )
    {
        s_recover_mode = true;
    }
    // maior que 70% não estamos em modo de recuperação.
    ...
    // Se estiver próximo ao gol, mudar
    else if (wm.self().pos().x > 30 && wm.ball().pos().x > 30){
        dlog.addText( Logger::TEAM,
                      __FILE__": Next to the goal.");
        // keep max power
        dash_power = ServerParam::i().maxDashPower();
    }
    // colocar o jogador para correr um pouco mais.
    else if(wm.self().pos().x < wm.ball().pos().x - 10 && wm.ball().pos().x > 15 && wm.self().pos().x < 20 && wm.self().pos().x > 0) {
        dlog.addText( Logger::TEAM,
                      __FILE__": Next to the goal.");
        // keep max power
    ...
    // dash power menor por estar no modo de recuperação. 
    else if ( s_recover_mode ) 
    {
        dash_power = my_inc - 25.0; // preffered recover value, mudar aqui para recuperar mais rápido.
        if ( dash_power < 0.0 ) dash_power = 0.0;

        dlog.addText( Logger::TEAM,
                      __FILE__": (get_normal_dash_power) recovering" );
    }
    // Se aproximar para tocar para outro companheiro de time.
    else if ( wm.existKickableTeammate()
              && wm.ball().distFromSelf() < 20.0 )
    {
        dash_power = std::min( my_inc * 0.9, //mudei aqui.
                               ServerParam::i().maxDashPower() );
        dlog.addText( Logger::TEAM,
                      __FILE__": (get_normal_dash_power) exist kickable teammate. dash_power=%.1f",
                      dash_power );
    }
    // Agente em área de impedimento.
    else if ( wm.self().pos().x > wm.offsideLineX() )
    {
        // Retirando um pouco da energia para sair da linha de impedimento.
        dash_power = ServerParam::i().maxDashPower() * 0.9;
        dlog.addText( Logger::TEAM,
